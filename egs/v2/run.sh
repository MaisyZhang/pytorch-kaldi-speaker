...
root=/home/pzhang/mydisk/kaldi_voxceleb
voxceleb1_trials=data/voxceleb1_test/trials
voxceleb1_root=/home/pzhang/mydisk/voxceleb1
voxceleb2_root=/home/pzhang/mydisk/voxceleb2
musan_root=/home/pzhang/mydisk/musan/musan

data=$root/data
exp=$root/exp
mfccdir=$root/mfcc
vaddir=$root/mfcc
nnet_dir=$exp/pytorch_xvector_nnet_1a_1111
...
if [ $stage -le 2 ]; then
  frame_shift=0.01
  awk -v frame_shift=$frame_shift '{print $1, $2*frame_shift;}' $data/train/utt2num_frames > $data/train/reco2dur  # 得到reco2dur文件
...
if [ $stage -le 3 ]; then
  ...
  # double the size of the original clean list. 将增强后的数据 + clean = 1,276,888 + 1000000
...
if [ $stage -le 4 ]; then
  ...
  # generate the spklist for train_combined_no_sil 
  awk -v id=0 '{print $1, id++}' $data/train_combined_no_sil/spk2utt > $data/train_combined_no_sil/spklist  
fi

# Training the network
if [ $stage -le 5 ]; then
    # KALDI-XVECTOR
    # local/nnet3/xvector/run_xvector.sh --stage $stage --data data/train_combined_no_sil --nnet-dir $nnet_dir \
    # --egs-dir $nnet_dir/egs

    nnet/run_train_nnet.sh --cmd "$train_cmd" --continue-training false nnet/nnet_conf/tdnn_xvector.json \
      $data/train_combined_no_sil $data/train_combined_no_sil/spklist \
      $nnet_dir    
fi
...
if [ $stage -le 6 ]; then
  ...
  nnet/run_extract_embeddings.sh --cmd "$train_cmd" --nj 10 --normalize false nnet/nnet_conf/tdnn_xvector.json \
    $nnet_dir $data/train $nnet_dir/xvectors_train
  
  nnet/run_extract_embeddings.sh --cmd "$train_cmd" --nj 10 --normalize false nnet/nnet_conf/tdnn_xvector.json \
    $nnet_dir $data/voxceleb1_test $nnet_dir/xvectors_voxceleb1_test
fi
...
#我的
# EER: 3.271%
# minDCF(p-target=0.01): 0.3397
# minDCF(p-target=0.001): 0.5630

# EER: 3.271%
# minDCF(p-target=0.01): 0.1787
# minDCF(p-target=0.001): 0.5630

# 清华的
# EER: 3.10%
# minDCF(p-target=0.01): 0.0169
# minDCF(p-target=0.001): 0.4977
